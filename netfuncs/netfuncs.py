...
def ipv4_to_value(ipv4_addr):
    ...
    """
    ips = [int(ip) for ip in ipv4_addr.split('.')]
    packed_ips = (ips[0] << 24) | (ips[1] << 16) | (ips[2] << 8) | ips[3]
    return packed_ips
...
def get_subnet_mask_value(slash):
    ...
    number = int(slash.split('/')[-1])
    return (0xffffffff << (32 - number)) & 0xffffffff
...
def ips_same_subnet(ip1, ip2, slash):
    ...
    ip1_packed = ipv4_to_value(ip1)
    ip2_packed = ipv4_to_value(ip2)
    subnet_mask = get_subnet_mask_value(slash)
    
    return (ip1_packed & subnet_mask) == (ip2_packed & subnet_mask)
...
def find_router_for_ip(routers, ip):
    ...
    for key in routers:
        router_ip = key
        router_netmask = routers[key]['netmask']
        if ips_same_subnet(ip, router_ip, router_netmask):
            matching_router = router_ip
...
def my_tests():
    ...
    print("-------------------------------------")
    testip = '127.0.0.1'
    packed_ip = ipv4_to_value(testip)
    unpacked_ip = value_to_ipv4(packed_ip)    
    print(testip)
    print(packed_ip)
    print(unpacked_ip)

    testsubnet = '127.0.0.1/16'
    print(get_subnet_mask_value(testsubnet))

    testsubnet2 = '127.9.2.123'
    testsubnet3 = '127.8.123.2'
    slash = '/8'
    slash1 = '/9'
    slash2 = '/16'
    slash3 = '/15'
    print(ips_same_subnet(testsubnet2, testsubnet3, slash))
    print(ips_same_subnet(testsubnet2, testsubnet3, slash1))
    print(ips_same_subnet(testsubnet2, testsubnet3, slash2))
    print(ips_same_subnet(testsubnet2, testsubnet3, slash3))
'''
